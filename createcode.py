...
def clientht2(rows, key):
    ...
    sb.append('    }'+'\n')
    sb.append('</style>'+'\n')
    sb.append('<link rel="stylesheet" href="~/Content/wangEditor/dist/css/wangEditor.min.css" />'+'\n')
    sb.append('<script src="~/Content/wangEditor/dist/js/wangEditor.min.js"></script>'+'\n')
    sb.append('<div id="app-view2" style=" display:none" v-show="isCreated">'+'\n')
    sb.append('    <el-button @@click="switchPage" type="primary" size="small">发布活动</el-button>'+'\n')
    sb.append('    <el-form ref="form" :model="queryPara" label-width="80px" inline style="text-align: left;float:right;">'+'\n')
    sb.append('        <div>'+'\n')
    sb.append('            <el-input v-model="queryPara.title" placeholder="请输入活动名称" style="width:200px;"></el-input>'+'\n')
    ...
    sb.append('        <el-table-column label="活动图" min-width="100" align="center">'+'\n')
    sb.append('            <template slot-scope="scope">'+'\n')
    sb.append('                <img :src="scope.row.Post" style="width:140px;max-height:120px;" />'+'\n')
    sb.append('            </template>'+'\n')
    sb.append('        </el-table-column>'+'\n')
    sb.append('        <el-table-column label="活动名称" prop="Title" min-width="100" align="center"></el-table-column>'+'\n')
    sb.append('        <el-table-column label="活动时间" min-width="100" align="center">'+'\n')
    sb.append('            <template slot-scope="scope">'+'\n')
    sb.append('                <span>{{convertData(scope.row.StartTime)}}</span>'+'\n')
    sb.append('                <br />/'+'\n')
    sb.append('                <br />'+'\n')
    sb.append('                <span>{{convertData(scope.row.EndTime)}}</span>'+'\n')
    sb.append('            </template>'+'\n')
    sb.append('        </el-table-column>'+'\n')
    sb.append('        <el-table-column label="人员限制" min-width="100" align="center">'+'\n')
    sb.append('            <template slot-scope="scope">'+'\n')
    sb.append('                <div v-if="scope.row.UserLimit > 0">'+'\n')
    sb.append('                    <span>{{scope.row.UserLimit}}人</span>'+'\n')
    sb.append('                    <br />'+'\n')
    sb.append('                    <span>（男：{{scope.row.MenNum}}；女：{{scope.row.WomenNum}}）</span>'+'\n')
    sb.append('                </div>'+'\n')
    sb.append('                <span v-else>无</span>'+'\n')
    sb.append('            </template>'+'\n')
    sb.append('        </el-table-column>'+'\n')
    sb.append('        <el-table-column label="参与人员" width="100" align="center">'+'\n')
    sb.append('            <template slot-scope="scope">'+'\n')
    sb.append('                <span v-if="scope.row.UserLimit > 0">{{scope.row.Applys}}/{{scope.row.UserLimit}}</span>'+'\n')
    sb.append('                <span v-else>{{scope.row.Applys}}</span>'+'\n')
    sb.append('                <br />'+'\n')
    sb.append('                <span class="el-button el-button--default el-button--mini" @@click="switchPage(2,scope.row.Id)">点击查看</span>'+'\n')
    sb.append('            </template>'+'\n')
    sb.append('        </el-table-column>'+'\n')
    sb.append('        <el-table-column label="活动类型" width="100" align="center">'+'\n')
    sb.append('            <template slot-scope="scope">'+'\n')
    sb.append('                <span>{{scope.row.Type == 1 ? "线上":"线下"}}</span>'+'\n')
    sb.append('            </template>'+'\n')
    sb.append('        </el-table-column>'+'\n')
    sb.append('        <el-table-column label="活动地址" width="150" align="center">'+'\n')
    sb.append('            <template slot-scope="scope">'+'\n')
    sb.append('                <span>{{scope.row.Citys}}</span>'+'\n')
    sb.append('            </template>'+'\n')
    sb.append('        </el-table-column>'+'\n')
    sb.append('        <el-table-column label="活动状态" width="100" align="center">'+'\n')
    sb.append('            <template slot-scope="scope">'+'\n')
    sb.append('                <span>{{scope.row.State == 1 ? "未发布": scope.row.State == 2 ? "已发布":"已结束"}}</span>'+'\n')
    sb.append('            </template>'+'\n')
    sb.append('        </el-table-column>'+'\n')
    sb.append('        <el-table-column label="操作" width="130" align="center">'+'\n')
    sb.append('            <template slot-scope="scope">'+'\n')
    sb.append('                <a class="el-button el-button--default el-button--mini btn-margin" :href=""/friendajax/exportformtoexcel/" + mid + "?aid=" + scope.row.Id" target="_blank">导出报名表单</a>'+'\n')
    sb.append('                <br />'+'\n')
    sb.append('                <div v-if="scope.row.State == 2">'+'\n')
    sb.append('                    <el-button class="btn-margin" size="mini" v-if="scope.row.IsTop == 0" @@click="sortActivity(scope.row.Id,1)">置顶活动</el-button>'+'\n')
    sb.append('                    <el-button type="primary" class="btn-margin" size="mini" v-if="scope.row.IsTop == 1" @@click="sortActivity(scope.row.Id,0)">取消置顶</el-button>'+'\n')
    sb.append('                    <br />'+'\n')
    sb.append('                </div>'+'\n')
    sb.append('                <el-button class="btn-margin" size="mini" @@click="switchPage(1,scope.row.Id)" v-if="scope.row.State != 2">编辑</el-button>'+'\n')
    sb.append('                <br v-if="scope.row.State != 2" />'+'\n')
    sb.append('                @*<el-button class="btn-margin" size="mini" @@click="switchPage(1,scope.row.Id)">编辑</el-button>'+'\n')
    sb.append('                <br />*@'+'\n')
    sb.append('                <el-button class="btn-margin" size="mini" v-if="scope.row.TemplateId < 1" @@click="setTemplate(scope.row.Id)">设为模板</el-button>'+'\n')
    sb.append('                <br v-if="scope.row.TemplateId < 1" />'+'\n')
    sb.append('                <el-button class="btn-margin" size="mini" @@click="delActivity(scope.row.Id)">删除</el-button>'+'\n')
    sb.append('            </template>'+'\n')
    sb.append('        </el-table-column>'+'\n')
    sb.append('    </el-table>'+'\n')
    sb.append(''+'\n')
    ...
    sb.append(''+'\n')
    sb.append('    @*弹窗 报名列表*@'+'\n')
    sb.append('    <el-dialog :center="true" title="报名表" :visible.sync="dialogVisible" width="70%">'+'\n')
    sb.append('        <div>           '+'\n')
    sb.append('            <el-row>'+'\n')
    ...
    sb.append('                </el-table>'+'\n')
    sb.append('            </el-row>'+'\n')
    sb.append('            <el-row style="text-align:center;">'+'\n')
    sb.append('                <el-pagination @@current-change="handleCurrentChange2"'+'\n')
    sb.append('                               :current-page="queryPara2.pageIndex"                               '+'\n')
    sb.append('                               :page-size="30"'+'\n')
    sb.append('                               layout="prev, next"'+'\n')
    sb.append('                               background>'+'\n')
    sb.append('                </el-pagination>'+'\n')
    sb.append('            </el-row>'+'\n')
    sb.append('        </div>'+'\n')
    ...
    sb.append('            <el-button type="primary" @@click="exportTable()" size="small">导出报名表</el-button>'+'\n')
    sb.append('        </div>'+'\n')
    sb.append('    </el-dialog>'+'\n')
    sb.append('</div>'+'\n')
    sb.append('<script type="text/javascript" src="/content/js/ajaxfileupload.js"></script>'+'\n')
    ...
    sb.append('                $.post("/friendajax/getactivitylist/" + this.mid, this.queryPara, function (res) {'+'\n')
    ...
    sb.append('                if (type == 1) {'+'\n')
    sb.append('                    location.href = "/friend/postactivity/" + this.mid + "?type=a&oid=" + id;'+'\n')
    sb.append('                } else if (type == 2) {'+'\n')
    sb.append('                    //location.href = "/friend/fapplyform/" + this.mid + "?aid=" + id;'+'\n')
    sb.append('                    this.queryPara2.aId = id;'+'\n')
    sb.append('                    this.loadData2();'+'\n')
    sb.append('                    this.dialogVisible = true;'+'\n')
    sb.append('                } else {'+'\n')
    sb.append('                    location.href = "/friend/postactivity/" + this.mid;'+'\n')
    sb.append('                }'+'\n')
    sb.append('            },'+'\n')
    sb.append('            handleClose(done) {'+'\n')
    sb.append('                this.$confirm("确认关闭？").then(_ => {done();}).catch(_ => { });'+'\n')
    sb.append('            },'+'\n')
    ...
    sb.append('            },'+'\n')
    sb.append('            sortActivity(id, sort) {'+'\n')
    sb.append('                $.post("/friendajax/sortactivity", { id: id, sort: sort, MinisnsId:@Model.Minisns.Id }, function (data) {'+'\n')
    sb.append('                    if (data.isok) {'+'\n')
    sb.append('                        var item = vm.list.find(function (o) {'+'\n')
    sb.append('                            return o.Id == id;'+'\n')
    sb.append('                        });'+'\n')
    sb.append('                        if (item) {'+'\n')
    sb.append('                            item.IsTop = sort;'+'\n')
    sb.append('                        }'+'\n')
    sb.append('                        vm.$message({'+'\n')
    sb.append('                            message: "操作成功",'+'\n')
    sb.append('                            type: "success"'+'\n')
    sb.append('                        });'+'\n')
    sb.append('                        vm.sortList(sort);'+'\n')
    sb.append('                    } else {'+'\n')
    sb.append('                        vm.$message(data.Msg);'+'\n')
    sb.append('                    }'+'\n')
    sb.append('                });'+'\n')
    sb.append('            },'+'\n')
    sb.append('            setTemplate(id) {'+'\n')
    sb.append('                $.post("/friendajax/setactivitytemplate/" + this.mid, { aid: id}, function (data) {'+'\n')
    sb.append('                    if (data.code == 1) {'+'\n')
    sb.append('                        var item = vm.list.find(function (o) {'+'\n')
    sb.append('                            return o.Id == id;'+'\n')
    sb.append('                        });'+'\n')
    sb.append('                        if (item) {'+'\n')
    sb.append('                            item.TemplateId = data.data;'+'\n')
    sb.append('                        }'+'\n')
    sb.append('                        vm.$message({'+'\n')
    ...
    sb.append('                        });'+'\n')
    sb.append('                    } else {'+'\n')
    sb.append('                        vm.$message("设置失败");'+'\n')
    sb.append('                    }'+'\n')
    sb.append('                });'+'\n')
    sb.append('            },'+'\n')
    sb.append('            sortList(type) {'+'\n')
    sb.append('                if (type == 1) {'+'\n')
    sb.append('                    this.list.sort(function (a, b) {'+'\n')
    sb.append('                        return b.IsTop - a.IsTop;'+'\n')
    sb.append('                    });'+'\n')
    sb.append('                } else {'+'\n')
    sb.append('                    this.list.sort(function (a, b) {'+'\n')
    sb.append('                        return b.Id - a.Id;'+'\n')
    sb.append('                    });'+'\n')
    sb.append('                }                '+'\n')
    sb.append('            },'+'\n')
    sb.append('            delActivity(id) {'+'\n')
    sb.append('                var item = this.list.find(function (o) {'+'\n')
    sb.append('                    return o.Id == id;'+'\n')
    sb.append('                });'+'\n')
    sb.append('                if (item) {'+'\n')
    sb.append('                    var str = "确定删除该活动?";'+'\n')
    sb.append('                    if (item.State == 2) {'+'\n')
    sb.append('                        str = "该活动正在进行中，确定删除该活动?";'+'\n')
    sb.append('                    } '+'\n')
    ...
    sb.append('                    }).then(() => {'+'\n')
    sb.append('                        $.post("/friendajax/deleteactivity/" + vm.mid, { oid: id }, function (data) {'+'\n')
    sb.append('                            if (data.isok) {'+'\n')
    sb.append('                                vm.$message({'+'\n')
    ...
    sb.append('            },'+'\n')
    sb.append('            loadData2() {'+'\n')
    sb.append('                this.loading2 = true;'+'\n')
    sb.append('                $.post("/friendajax/getactivityapplylist/" + this.mid, this.queryPara2, function (res) {'+'\n')
    sb.append('                    if (res.code === 1) {'+'\n')
    sb.append('                        if (vm.queryPara2.pageIndex === 1) {'+'\n')
    sb.append('                            vm.list2 = [];'+'\n')
    sb.append('                        }'+'\n')
    sb.append('                        vm.list2 = vm.list2.concat(res.data.list || []);'+'\n')
    sb.append('                    } else {'+'\n')
    sb.append('                        vm.$message({ showClose: true, message: res.msg, type: "info" });'+'\n')
    sb.append('                    }'+'\n')
    sb.append('                    vm.loading2 = false;'+'\n')
    sb.append('                });'+'\n')
    sb.append('            },'+'\n')
    sb.append('            handleCurrentChange2(pageIndex) {'+'\n')
    sb.append('                if (pageIndex == this.queryPara2.pageIndex) return;'+'\n')
    sb.append('                this.list2 = [];'+'\n')
    sb.append('                this.queryPara2.pageIndex = pageIndex;'+'\n')
    sb.append('                this.loadData2();'+'\n')
    sb.append('            },'+'\n')
    sb.append('            exportTable() {'+'\n')
    sb.append('                //window.location.href = "/friendajax/exportformtoexcel/" + this.mid + "?aid=" + this.queryPara2.aId;'+'\n')
    sb.append('                window.open("/friendajax/exportformtoexcel/" + this.mid + "?aid=" + this.queryPara2.aId);'+'\n')
    sb.append('            }'+'\n')
    sb.append('        },'+'\n')
    sb.append('        created() {'+'\n')
